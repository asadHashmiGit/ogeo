	...
	<div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4>Rate Contract Acceptance List</h4>
                </div>
                <div class="card-body">
                    <table ref="VendorRCsList" id="VendorRCsList" class="display responsive table table-bordered table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>RC #</th>
                                <th>Company Name</th>
                    ...
                    <div class="modal-body" v-if="editDataLoaded">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="table-wrapper">
                                    <table ref="RCDetailsTable" id="RCDetailsTable" class="table table-bordered" style="width:100%">
                                        ...
                                        <tbody>
                                            <tr v-for="(RCResponse, index) in EditModalInfo.vendor_responses">

                                                <td v-if="RCResponse.rate_contract_request.item_id" style="min-width: 150px;">{{ RCResponse.rate_contract_request.item.description }}</td>
                                                <td v-else style="min-width: 500px;">{{ RCResponse.rate_contract_request.service_description }}</td>

                                                <td>{{ RCResponse.rate_contract_request.quantity }}</td>
                                                <td>{{ RCResponse.unit_rate }}</td>
                                                <td>{{ parseFloat(RCResponse.unit_rate) * parseFloat(RCResponse.rate_contract_request.quantity) }} </td>
                                                <td>{{ RCResponse.currency }} </td>
                                                <td style="min-width: 150px">
                                                    <span><b>Lead Days:</b> {{ RCResponse.lead_days }} Days</span><br>
                                                    <span><b>Advanced Percentage:</b> {{ RCResponse.advanced_percentage }}%</span><br>
                                                    <span v-if="RCResponse.notes">Vednor Notes: 
                                                        <span>{{ RCResponse.notes }}</span>
                                                    </span>
                                                </td>
                            ...
                            <div class="col-lg-12">
                            	<hr>
                            	<el-input
                                   :id="'RC_'+EditModalInfo.id+'_Vendor_Decision_Notes'"
                                   style="min-width: 100%"
                                   type="textarea"
                                   rows="3"
                                   placeholder="Decision Notes">
                                </el-input><br><br>
                            	<button @click="RejectRC(EditModalInfo.id)" style="width: 45%" type="button" class="btn btn-danger pull-left">Reject</button> 
                                <button @click="ApproveRC(EditModalInfo.id)" style="width: 45%" type="button" class="btn btn-success pull-right">Approve</button>
                            </div>
                        </div>
        ...
        <div id="RateContractValidationModalView" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="RateContractValidationModalView" aria-hidden="true" style="display: none;">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div  class="modal-header">
                        <h4 class="modal-title">Rate Contract Number: {{ ViewModalInfo.id }}</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body" v-if="viewDataLoaded">
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="table-wrapper">
                                    <table ref="RCViewDetailsTable" id="RCViewDetailsTable" class="table table-bordered" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th style="min-width: 150px;">Item Details</th>
                                                <th>Quantity</th>
                                        ...
                                        <tbody>
                                            <tr v-for="(RCResponse, index) in ViewModalInfo.vendor_responses">
                                                <td v-if="RCResponse.rate_contract_request.item_id" style="min-width: 150px;">{{ RCResponse.rate_contract_request.item.description }}</td>
                                                <td v-else style="min-width: 500px;">{{ RCResponse.rate_contract_request.service_description }}</td>
	...
	export default {
		...
		methods: {
			ApproveRC(RCId){
                ...
                axios.post('/api/vendors/pos_n_invoicing_management/rc_vendor_approval', {RCId: RCId, VendorNotes: $('#RC_'+RCId+'_Vendor_Decision_Notes').val()})
                        .then((response) => {
                            Swal({ 
			...
			RejectRC(RCId){
				...
				if($('#RC_'+RCId+'_Vendor_Decision_Notes').val() == ""){
                    Swal('Notes Required', 'Rejection Require Notes', 'warning');
                } else {

					axios.post('/api/vendors/pos_n_invoicing_management/rc_vendor_rejection', {RCId: RCId, VendorNotes: $('#RC_'+RCId+'_Vendor_Decision_Notes').val()})
	                    .then((response) => {
	                    	Swal({ 
                                type: response.data.messageType, 
                                title: response.data.messageTitle, 
			...
			showViewModal(recordId){
				this.ViewModalInfo = {};
                axios.post('/api/vendors/data/get_vendor_rc_details', {RCId: recordId})
                    .then((response) => {
                        ...
                        this.$nextTick(() => {
                            var table = $('#RCViewDetailsTable').DataTable({
                                colReorder: true,
		...
		mounted(){
			...
			const table = $('#VendorRCsList').DataTable({
                stateSave:  true,
                responsive: true,
                colReorder: true,
                processing: true,
                serverSide: true,
                ajax: 'http://' + window.location.host + '/api/vendors/data/get_vendor_rc_list',
                columns: [
                    { data: 'id', name: 'id' },
            ...
            $('tbody', this.$refs.VendorRCsList).on('click', '.view-placeholder', function(){
                var $this = $(this);
                var RecordID = $(this).data('recordid');
                self.showViewModal(RecordID);
            });

            $('tbody', this.$refs.VendorRCsList).on('click', '.edit-placeholder', function(){
                var $this = $(this);
                var RecordID = $(this).data('recordid');
