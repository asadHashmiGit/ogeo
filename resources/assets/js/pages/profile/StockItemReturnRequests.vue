<template>
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h4>Stock Item Return Requests</h4>
                </div>
                <div class="card-body">
                    <table ref="table" id="StockItemReturnRequestList" class="responsive table table-bordered table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>Request #</th>
                                <th>Created By</th>
        ...
        <div id="StockItemReturnRequestModalView" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="StockItemReturnRequestModalView" aria-hidden="true" style="display: none;">
    ...
    export default {
        ...
        methods: {
            showViewModal(elquentClass, recordId){
                let self = this;
                this.ViewModalInfo = {};
                axios.post('/api/users/get_user_stock_item_return_request_record_details', [elquentClass, recordId])
                    .then((response) => {
                        this.ViewModalInfo = response.data;
                        this.viewDataLoaded = true
                        $('#StockItemReturnRequestModalView').modal('toggle');
                    })
        ...
        mounted(){
            ...
            const table = $('#StockItemReturnRequestList').DataTable({
                stateSave:  true,
                responsive: true,
                colReorder: true,
                processing: true,
                serverSide: true,
                ajax: 'http://' + window.location.host + '/api/users/get_users_stock_return_requests',
                columns: [
...
<style scoped>
    ...
    #StockItemReturnRequestModalView .modal-dialog {
        min-width:80%;
    }
